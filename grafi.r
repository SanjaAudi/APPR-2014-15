...
g1 <- g1[!is.na(g1)]
pdf("slike/Audi_trgi_1996_1997.pdf")
...
barplot(f1/1000,main='Zaposleni v Audi tovarnah 2000', ylab='Število zaposlenih (×1000)', col=c("pink"),las=2,cex.names=0.6)
dev.off()

f2 <- apply(AudiEmployees,1,function(x) x[3])
pdf("slike/Audi_zaposleni_2002.pdf")
barplot(f2/1000,main='Zaposleni v Audi tovarnah 2002', ylab='Število zaposlenih (×1000)', col=c("blue"),las=2,cex.names=0.6)
dev.off()

f3 <- apply(AudiEmployees,1,function(x) x[4])
pdf("slike/Audi_zaposleni_2003.pdf")
barplot(f3/1000,main='Zaposleni v Audi tovarnah 2003', ylab='Število zaposlenih (×1000)', col=c("yellow"),las=2,cex.names=0.6)
dev.off()

f4 <- apply(AudiEmployees,1,function(x) x[5])
pdf("slike/Audi_zaposleni_2004.pdf")
barplot(f4/1000,main='Zaposleni v Audi tovarnah 2004', ylab='Število zaposlenih (×1000)', col=c("green"),las=2,cex.names=0.6)
dev.off()
